...
page_init(void) {
    ...
    // EXTPHYSMEM is the address where kern starts, we need the address where kern ends
    // we can get this by allolcate(0) when kern starting while the physical memory has
    // not been allocated
    size_t io_hole_end_page = PADDR(boot_alloc(0)) / PGSIZE;
    for (i = 0; i < npages; i++) {
        // idt
        if (i == 0) {
            pages[0].pp_ref = 1;
...
page_alloc(int alloc_flags) {
    ...
    if (result == NULL) {
        cprintf("page allocate fail! not enough memory!");
        return result;
    }
...
pgdir_walk(pde_t *pgdir, const void *va, int create) {
    ...
    if (!(*pte_ptr & PTE_P)) {
        if (create) {
            ...
            pageInfo_ptr->pp_ref = 1;
            *pte_ptr = page2pa(pageInfo_ptr) | PTE_P | PTE_U | PTE_W;
        } else {
            return NULL;
        }
    }
